...
class JDEVEMAnalyser(JDEAnalyser):
    """Class that handles parcel-wise which is done according to the input data parcellation by default, and also takes
    care of merging parcel-specific outputs at the end of the JDE VEM analysis.
    ...
    Args:
        ...
        contrasts (:obj:`~collections.OrderedDict`): contrasts to be evaluated. 
    ...
    parametersComments = {
        ...
        'scale': 'if true, the data fidelity term is divide by the number of voxels, otherwise it does nothing',
        ...
        'HrfFilename': 'HRF filename',
        'estimateBeta': 'estimate or not the Potts spatial regularization parameter',
        'estimateDrifts': 'explicit drift estimation (if false, drifts are marginalized',
        'drifts_type': 'type of the drift basis (default="polynomial")',
        ...
        'LabelsFilename': 'labels filename',
        ...
        'InitVar': 'initial value of active and inactive gaussian variances',
        'InitMean': 'initial value of active gaussian means',
        'MiniVemFlag': 'if true, estimate the best initialisation of MixtParam and gamma_h',
        'NbItMiniVem': 'number of iterations in Mini VEM algorithm',
    ...
    def __init__(self, hrfDuration=25., sigmaH=0.1, fast=True,
        ...
        self.contrasts = OrderedDict() if contrasts is None else contrasts
    ...
    def analyse_roi(self, roiData):
        ...
        Args:
            roiData (:obj:`~pyhrf.core.FmriData`): fMRI data to be analyzed.  

        Returns:
            :obj:`dict` of :obj:`~pyhrf.ndarray.xndarray`: packed outputs.
        ...
        scale = nvox if self.scale else 1
        rid = roiData.get_roi_id()
        logger.info("JDE VEM - roi %d, nvox=%d, nconds=%d, nItMax=%d", rid, nvox, len(Onsets), self.nItMax)
        ...
        if self.fast:
            logger.info("fast VEM with drift estimation" + ("and a constraint"*self.constrained))

            (nb_iter, nrls_mean, hrf_mean, hrf_covar, labels_proba, noise_var, nrls_class_mean, nrls_class_var, beta,
             drift_coeffs, drift, contrasts_mean, contrasts_var, _, _, nrls_covar, _, density_ratio,
             density_ratio_cano, density_ratio_diff, density_ratio_prod, ppm_a_nrl, ppm_g_nrl, ppm_a_contrasts,
             ppm_g_contrasts, variation_coeff, free_energy, free_energy_crit, beta_list, delay_of_response,
             delay_of_undershoot, dispersion_of_response, dispersion_of_undershoot, ratio_resp_under,
             delay) = jde_vem_bold(graph, data, Onsets, durations, self.hrfDuration, self.nbClasses, TR, self.beta,
                                   self.dt, self.estimateSigmaH, self.sigmaH, self.nItMax, self.nItMin,
                                   self.estimateBeta, self.contrasts, self.computeContrast, self.hyper_prior_sigma_H,
                                   self.estimateHRF, constrained=self.constrained, zero_constraint=self.zero_constraint,
                                   drifts_type=self.drifts_type)

        elif self.estimateDrifts:  # if not self.fast
            logger.info("not fast VEM")
            logger.info("NOT WORKING")
            (nrls_mean, hrf_mean, labels_proba, noise_var, nrls_class_mean, nrls_class_var, beta, drift_coeffs,
             drift) = Main_vbjde_Python_constrained(graph, data, Onsets, self.hrfDuration, self.nbClasses, TR,
                                                    self.beta, self.dt, scale, self.estimateSigmaH, self.sigmaH,
                                                    self.nItMax, self.nItMin, self.estimateBeta, self.PLOT)

        # Plot analysis duration
        self.analysis_duration = time() - t_start
        logger.info('JDE VEM analysis took: %s', format_duration(self.analysis_duration))
        ...
        if self.fast:
            ...
            if self.estimateHRF:  # if self.fast
                fitting_parameters = {
            ...
            if (len(self.contrasts) > 0) and self.computeContrast:
                ...
                outputs['contrasts_variance'] = xndarray(contrasts_var, value_label="Contrasts_Variance",
                                                         axes_names=['voxel', 'contrast'],
                                                         axes_domains=domContrast)
        ...
        if self.simulation and self.fast:
            ...
            for i in xrange(0, M):
                se0, sp0, auc = roc_curve(newlabels[i, :].tolist(), true_labels[i, :].tolist())
                se.append(se0)
                sp.append(sp0)
            ...
            outputs['ROC_audio'] = xndarray(sensData, axes_names=axes_names,
                                            axes_domains=ad,
            ...
            plt.figure(200)
            plt.plot(sensData[0], specData[0], '--', color='k', linewidth=2.0, label='m=1')
            plt.hold(True)
            plt.plot(sensData[1], specData[1], color='k', linewidth=2.0, label='m=2')
            # plt.legend(('audio','video'))
            ...
            if hasattr(roiData.simulation, 'nrls'):
                ...
                raise Exception('Simulation can not be retrieved from %s' % str(roiData.simulation))
