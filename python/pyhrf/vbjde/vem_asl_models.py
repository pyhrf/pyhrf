...
def Main_vbjde_physio(graph, Y, Onsets, durations, Thrf, K, TR, beta, dt,
    ...
    # Initialization
    gamma_h = 100000000  # 10000000000  # 7.5
    ...
    beta = 1.
    Thresh = 1e-5
    print 'Ncoefs = ', Thrf / dt
    print 'HRF duration = ', Thrf
    print 'dt = ', dt
    ...
    while ((ni < NitMin + 1) or (((Crit_AH > Thresh) or (Crit_CG > Thresh)) \
        ...
        if estimateH:
            ...
            #tA = time.time()
            Ht, Sigma_H1 = EM.expectation_H_prior_copy(Sigma_A, m_A, m_C, G, X, W,
                                                  Gamma, D, J, N, y_tilde,
                                                  sigma_eps, scale, matrix_cov,
                                                  sigmaH, sigmaG, priorH_mean_term,
                                                  priorH_cov_term)
            #print 'm_H = ', Ht.shape
            #print 'Sigma_H = ', Sigma_H1.shape
            #tB = time.time()
            #print 'time version slow = ', tB - tA
            """
            tB = time.time()
            Ht1, Sigma_H = EM.expectation_H_prior(Sigma_A, m_A, m_C, G, XX, W,
                                                  Gamma, D, J, N, y_tilde,
                                                  sigma_eps, scale, matrix_cov,
                                                  sigmaH, sigmaG, priorH_mean_term,
                                                  priorH_cov_term)
            print 'time version fast = ', time.time() - tB
            print np.sum((Ht1 - Ht)**2)
            print np.sum((Sigma_H1 - Sigma_H)**2)
            """
            #stop
            
            if constraint and not np.linalg.norm(Ht)==1:
    ...
    if PLOT:
        ...
        EM.plot_convergence(ni, M, cA, cC, cH, cG, cAH, cCG, SUM_q_Z, mua1, muc1)
