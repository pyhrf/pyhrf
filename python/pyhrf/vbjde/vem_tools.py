...
def ppms_computation(elements_mean, elements_var, class_mean, class_var, threshold_a="std_inact",
    ...
    if threshold_a == "std_inact":
        ...
        print thresh
        print thresh.shape
        print elements_mean.shape
        print elements_var.shape
...
def maximization_sigma_asl(D, Sigma_H, R_inv, m_H, use_hyp, gamma_h, haux=0):
    alpha = (np.dot(m_H[:, np.newaxis] * m_H[np.newaxis, :] + Sigma_H, R_inv)+haux).trace()
    if use_hyp:
