...
def Main_vbjde_Extension_constrained(graph,Y,Onsets,Thrf,K,TR,beta,dt,scale=1,estimateSigmaH=True,sigmaH = 0.05,NitMax = -1,NitMin = 1,estimateBeta=True,PLOT=False,contrasts=[],computeContrast=False,gamma_h=0,estimateHRF=True,TrueHrfFlag=False,HrfFilename='hrf.nii',estimateLabels=True,LabelsFilename='labels.nii',MFapprox=False,InitVar=0.5,InitMean=2.0,MiniVEMFlag=False,NbItMiniVem=5):    
    ...
    if PLOT:
        ...
        plt.savefig('./HRF_Iter_CompMod_C.png')
        plt.hold(False)
        plt.figure(2)
        ...
        plt.legend( ('CAH', 'CFE') )
        plt.grid(True)
        plt.savefig('./Crit_CompMod_C.png')
        plt.figure(3)
        plt.plot(FreeEnergyArray)
        plt.grid(True)
        plt.savefig('./FreeEnergy_CompMod_C.png')
        ...
        #plt.legend( ('m=0','m=1') ) 
        plt.savefig('./Sum_q_Z_Iter_CompMod_C.png')
        ...
        for m in xrange(M):
            ...
            plt.hold(True)
        plt.hold(False)
        plt.savefig('./mu1_Iter_CompMod_C.png')
        
        plt.figure(6)
        plt.plot(h_norm_array)
        plt.savefig('./HRF_Norm_CompMod_C.png')
        
        Data_save = xndarray(h_norm_array, ['Iteration'])
        Data_save.save('./HRF_Norm_Comp_C.nii')        
    ...
    pyhrf.verbose(1, "Parameters")
    pyhrf.verbose(1, "    mu_M:" + str(int(mu_M) ))
    pyhrf.verbose(1, "    sigma_M:" + str(int(sigma_M) ))
    pyhrf.verbose(1, "    sigma_H:" + str(sigma_H) )
    pyhrf.verbose(1, "    mu_M:" + str(int(mu_M) ))
