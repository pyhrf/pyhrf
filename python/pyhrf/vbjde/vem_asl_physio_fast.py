...
def Main_vbjde_physio(graph, Y, Onsets, durations, Thrf, K, TR, beta, dt,
    ...
    W = np.diag(w)
    # Conditions
    X, XX, conditions_names = EM.create_conditions_block(Onsets, durations, M, N, D, TR, dt)
    #X, XX, conditions_names = EM.create_conditions(Onsets, M, N, D, TR, dt)
    ...
    while ((ni < NitMin + 1) or ((Crit_AH > Thresh) and (Crit_CG > Thresh) \
        ...
        if estimateH:
            ...
            Ht, Sigma_H = EMf.expectation_H(Sigma_A, m_A, m_C, G, X, W,
                                                  Gamma, D, J, N, y_tilde,
        ...
        if estimateG:
            ...
            Gt, Sigma_G = EMf.expectation_G(Sigma_C, m_C, m_A, H, XX, W, 
                                            Gamma, D, J, N, y_tilde, sigma_eps,
                                            scale, R_inv, sigmaG, priorG_mean_term, priorG_cov_term)
            if 1:  #normg1:
                logger.info("constraint l2-norm = 1")
        ...
        if estimateA:
            logger.info("estimation")
            m_A, Sigma_A = EMf.expectation_A(H, m_A, G, m_C, W, X, Gamma, q_Z,
        ...
        if estimateC:
            ...
            m_C, Sigma_C = EMf.expectation_C(G, m_C, H, m_A, W, XX, Gamma, q_Z,
                                             mu_Mc, sigma_Mc, D, J, M, K,
        ...
        if estimateLA:
            ...
            L, alpha = EMf.maximization_LA(Y, m_A, m_C, XX, w, W, H, G, P, Gamma)
        ...
        if estimateBeta:
            ...
            for m in xrange(0, M):
                ...
                Beta[m] = EMf.maximization_beta_m4(Beta[m].copy(), q_Z[m, :, :],
        ...
        if estimateNoise:
            ...
            sigma_eps = EMf.maximization_sigma_noise(XX, m_A, Sigma_A, H, \
                                                     m_C, Sigma_C, G, \
                                                     Sigma_H, Sigma_G, W, y_tilde)
