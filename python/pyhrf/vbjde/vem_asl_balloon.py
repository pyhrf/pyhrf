...
def Main_vbjde_physio(graph, Y, Onsets, durations, Thrf, K, TR, beta, dt,
    ...
    Hb = create_physio_brf(phy_params, response_dt=dt, response_duration=D-1)
    print 'D = ', D
    print 'H_physio shape = ', Hb.shape
    Gb = create_physio_prf(phy_params, response_dt=dt, response_duration=D-1)
    print 'G_physio shape = ', Gb.shape
    H = copy.deepcopy(H)
    H1 = copy.deepcopy(H)
    Ht = copy.deepcopy(H)
