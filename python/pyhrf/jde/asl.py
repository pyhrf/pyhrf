...
class DriftCoeffSampler(GibbsSamplerVariable, xmlio.XMLable2):
    ...
    def sampleNextInternal(self, variables):
        ...
        for i in xrange(self.nbVoxels):
            ...
            pyhrf.verbose(5, 'ivox=%d, v_lj=%f, std_lj=%f mu_lj=' \
                          %(i,v_lj,v_lj**.5), mu_lj)
...
class BOLDResponseSampler(ResponseSampler, xmlio.XMLable2):
...
class PerfResponseSampler(ResponseSampler, xmlio.XMLable2):
        ...
        if self.dataInput.simulData is not None:
            ...
            if not brl_sampler.sampleFlag and not brf_sampler.sampleFlag and\
              brl_sampler.useTrueValue and brf_sampler.useTrueValue:
                assert_almost_equal(sumaXh, sd['bold_stim_induced'][::osf])
            ...
            if not brl_sampler.sampleFlag and not brf_sampler.sampleFlag and\
              ...
              perf_baseline_sampler.useTrueValue:
                ...
                perf = np.dot(self.dataInput.W, sd['perf_stim_induced'][::osf])
...
class BOLDResponseVarianceSampler(ResponseVarianceSampler, xmlio.XMLable2):
...
class PerfResponseVarianceSampler(ResponseVarianceSampler, xmlio.XMLable2):
        ...
        if self.dataInput.simulData is not None:
            ...
            if not brf_sampler.sampleFlag and brf_sampler.useTrueValue and\
              ...
              not drift_sampler.sampleFlag and drift_sampler.useTrueValue:
                ...
                osf = int(sd['tr'] / sd['dt'])
                perf = np.dot(self.dataInput.W, sd['perf_stim_induced'][::osf])
                true_res = sd['bold'] - sd['bold_stim_induced'][::osf] - \
                  perf - sd['drift']
