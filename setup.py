...
def parse_setuppy_commands():
    ...
    commands.  Return a boolean value for whether or not to run the
    build or not (avoid installing numpy)
    """
    if len(sys.argv) < 2:
        # User forgot to give an argument probably, let setuptools handle that.
        return True

    good_commands = ('develop', 'install', 'sdist', 'build', 'build_ext', 'build_py',
