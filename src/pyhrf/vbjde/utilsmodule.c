...
for (d=0;d<Nrep;d++){
  for (nr=0;nr<Nrep;nr++){
    /* GGamma[d*Nrep + nr] = GetValue(Gammaarray,nr,d); */
    GGamma[nr*Nrep + d] = GetValue(Gammaarray,nr,d);
  }
}
/* printf("Gamma[10, 20] %f\n", GGamma[10*Nrep + 20]); */
for (d=0;d<D;d++){
  H[d] = GetValue1D(m_Harray,d);
  for (nr=0;nr<D;nr++){
      SSigma_H[d*D+nr] = GetValue(Sigma_Harray,d,nr);
  ...
  for (m2=0;m2<nCond;m2++){
    for (d=0;d<D;d++){
      for (nr=0;nr<Nrep;nr++){
        XX[nr*D + d] = GetValue3DInt(Xarray,m2,nr,d);
        XXT[d*Nrep + nr] = GetValue3DInt(Xarray,m,nr,d);
        /* printf("m=%d, m2=%d, d=%d, nr=%d, XX=%d\n", m, m2, d, nr, XX[nr*D + d]); */
        /* printf("m=%d, m2=%d, d=%d, nr=%d, XXT=%d\n", m, m2, d, nr, XXT[d*Nrep + nr]); */
      }
    }
    prodMat4(H,XXT,tmpNrep,1,Nrep,D);
    /* printf("HX %f\n", tmpNrep[0]); */
    ...
    prodMat4(tmpNrep2,XX,tmpD,1, D, Nrep);
    Sigma_A0[m*nCond + m2] = prodMatScal(tmpD,H,D);
    /* printf("m=%d, m2=%d, before trace %f\n", m, m2, Sigma_A0[m*nCond + m2]); */
    prodMat4(SSigma_H,XXT,tmp,D,Nrep,D);
    /* printf("SX %f\n", tmp[0]); */
    prodMat2(tmp,GGamma,tmpT,D,Nrep,Nrep);
    /* printf("SXG %f\n", tmpT[0]); */
    ...
    /* printf("tr(SXGX) %f\n", traceMat(tmpDD, D)); */
    Sigma_A0[m*nCond + m2] += traceMat(tmpDD,D);
    /* printf("after trace m=%d, m2=%d, %f\n", m, m2, Sigma_A0[m*nCond + m2]); */
    Delta[m*nCond+m2] = 0;
  }
}
/* test = PyArray_MatrixProduct(Gamma, y_tilde); */
/* printf("%f\n", *(npy_float64 *) PyArray_GETPTR2(test, 0, 0)); */
/* test = PyArray_MatrixProduct(PyArray_Transpose(PyArray_MatrixProduct(Gamma, y_tilde), NULL), X); */
/* for (j=0; j<J; j++){ */
    /* printf("j=%2d, GYtX from numpyprod %f\n", j, *(npy_float64 *) PyArray_GETPTR3(test, j, 0, 0)); */
/* } */
for (j=0;j<J;j++){
  /* if (GetValue1D(sigma_epsilonearray,j) < eps) GetValue1D(sigma_epsilonearray,j) = eps; */
  for (nr=0;nr<Nrep;nr++){
  ...
  for (m=0;m<nCond;m++){
      ...
      prodMat4(tmpNrep,XX,tmpD,1,D,Nrep);
      /* if (m==0){ */
          /* printf("j=%2d, GYtX from prodmat   %f\n", j, tmpD[0]); */
      /* } */
      for (d=0;d<D;d++){
        X_tilde[m*D+d] = tmpD[d] / GetValue1D(sigma_epsilonearray,j);
      }
...
/* static PyObject *UtilsC_hrf_expectation(PyObject *self, PyObject *args){ */

    /* [> PyObject *XGamma,*Y,*y_tilde,*m_A,*m_H,*X,*R,*Gamma,*Sigma_A,*sigma_epsilone,*Sigma_H,*QQ_barnCond; <] */
    /* PyObject *nrls_covar, *nrls_mean, *occurence_matrix, *noise_struct, *hrf_regu_prior_inv, *sigmaH, *nb_voxels, *y_tilde, *noise_var, *prior_mean_term, *prior_cov_term; */
    ...
    /* PyArg_ParseTuple(args, "OOOOOdiOOdd",&nrls_covar,&nrls_mean,&occurence_matrix,&noise_struct,&hrf_regu_prior_inv,&sigmaH,&nb_voxels,&y_tilde,&noise_var,&prior_mean_term,&prior_cov_term); */
    /* PyObject *nm_om_prod, *ns_om_prod, *om_ns_om_prod, *cov_noise, *nm_om_ns_prod, *hrf_covar_inv, *hrf_covar, *hrf_mean; */
    /* PyArrayObject **einsteinsumarray; */
    /* [> Xarray   = (PyArrayObject *) PyArray_ContiguousFromObject(X,PyArray_INT,3,3); <] */
    /* [> m_Aarray = (PyArrayObject *) PyArray_ContiguousFromObject(m_A,PyArray_FLOAT64,2,2); <] */
    ...
    /* einsteinsumarray = malloc(sizeof(*PyArrayObject)*2); */

    /* nm_om_prod = PyArray_MatrixProduct(nrls_mean, occurence_matrix); */
    /* ns_om_prod = PyArray_InnerProduct(noise_struct, occurence_matrix); */
    /* om_ns_om_prod = PyArray_MatrixProduct(occurence_matrix, ns_om_prod); */
    /* cov_noise = PyArray_MAX(noise_var, eps); */

    /* iter1 = (PyArrayIterObject *)PyArray_IterNew(hrf_regu_prior_inv); */
    /* iter2 = (PyArrayIterObject *)PyArray_IterNew(hrf_covar_inv); */
    /* while (iter1->index < iter1->size) { */
        /* iter2->dataptr = iter1-> dataptr * sigmaH; */
        /* PyArray_ITER_NEXT(iter); */
    /* } */

    /* einsteinsumarray[0] = nrls_covar; */
    /* einsteinsumarray[1] = om_ns_om_prod; */
    /* tmp = PyArray_EinsteinSum("ijk,lijm->klm", 2, einsteinsumarray); */
    /* iter = (PyArrayMultiIterObject *)PyArray_MutliIterNew(2, tmp, cov_noise); */
    /* while (iter->index < iter->size) { */
        /* iter->iters[0] /= iter->iters[1] */
    /* } */
    /* PyArray_Sum(tmp, 0, rtype, tmp); */
